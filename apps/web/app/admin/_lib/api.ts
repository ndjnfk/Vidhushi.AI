...
import { makeCallApi, type ConsultationRequestOut } from "@/lib/bookings";
...
export const listBookings = (status?: string) =>
  adminFetch<ConsultationRequestOut[]>(`/admin/bookings${status ? `?status=${status}` : ""}`);
export const approveBooking = (
  id: string,
  body: { scheduled_at: string; duration_minutes: number; amount: number; note: string },
...
export const bookingCounts = () => adminFetch<{ pending: number; payment_submitted: number }>("/admin/bookings/counts");
